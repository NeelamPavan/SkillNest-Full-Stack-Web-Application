...
const Header = () => {
    ...
    return( 
    <motion.header layout variants={navbarVariants} initial={["default",{y:-100}]} animate={[scrolled?"active": "default","slide"]} transition={{duration:0.3}} className="fixed flex items-center h-[70px] z-[1000] w-full border border-solid border-transparent">
...
<button type="button" className="btn btn-outline">Sign Up</button></a>
<a href="http://localhost:3000/home" target="_blank" rel="noopener noreferrer">
<button type="button" className="btn btn-primary">Log In</button></a>
</div>
     </div>
     <div className={`header__menu-icon flex lg:hidden relative w-5 h-4 flex-shrink-0 cursor-pointer overflow-hidden ${toggleMenu? "active":""}`}onClick={()=> setToggleMenu(!toggleMenu)}>
        <span></span>
        <span></span>
        <span></span>
     </div>
   ...
   {toggleMenu &&(
    ...
    variants={mobileMenuVariants}initial="hidden" animate="visible" exit="hidden">
        <div className="container flex flex-col justify-between h-full pt-[100px] pb-6">
            ...
            {navLinks.map((link)=>{
          return( <li key={link.id} className="h-8">
